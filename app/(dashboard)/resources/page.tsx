...
import {
  Plus, Trash2, ExternalLink, Youtube, FileText, Wrench, CalendarDays, X,
} from "lucide-react";
...
export default function LearningPage() {
  ...
  // 15-day chart data (uses createdAt)
  ...
  const chartData = chart15Days.map((date) => {
    ...
    RESOURCE_CATEGORIES.forEach((cat) => {
      entry[cat] = resources.filter((r) => {
        ...
        return created === date && r.category === cat;
      }).length;
    });
    return entry;
  });
  ...
  const activeCategoriesInChart = RESOURCE_CATEGORIES.filter((cat) =>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {/* 15-day learning activity chart */}
      ...
      {clickedDate &&
        (() => {
          ...
          const byCategory = RESOURCE_CATEGORIES.map((cat) => ({
            category: cat,
            items: dayItems.filter((r) => r.category === cat),
          })).filter((g) => g.items.length > 0);
